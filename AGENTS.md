...
- Codex model capabilities are not fully advertised by the current `model/list` metadata; when `turn/start` rejects `reasoning.summary` with `unsupported_parameter`, retry without that field and cache the result per model/default-model path instead of hardcoding model-name exceptions.
